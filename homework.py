...
def send_message(bot, message):
    ...
    try:
        ...
        logger.error(f'Ошибка [{error}]. Сообщение "{message}",'
...
def get_api_answer(current_timestamp):
    ...
    if status == 200:
        ...
        raise Exception(f'Ошибка запроса к API-сервису [{ENDPOINT}]:'
...
def parse_status(homework):
    ...
    if (verdict is None):
        raise Exception('Ошибка: В ответе API, обнаружен недокументированный'
...
def check_tokens():
    ...
    if not TELEGRAM_TOKEN:
        ...
        logger.debug('Переменная окружения TELEGRAM_TOKEN загружена.')
        if not TELEGRAM_CHAT_ID:
            logger.critical('Отсутствует переменная окружения'
                            'TELEGRAM_CHAT_ID!')
        else:
            logger.debug('Переменная окружения TELEGRAM_CHAT_ID загружена.')
            if not PRACTICUM_TOKEN:
                logger.critical('Отсутствует переменная окружения'
                                'PRACTICUM_TOKEN!')
            else:
                logger.debug('Переменная окружения PRACTICUM_TOKEN загружена.')
                return True
    return False
    ...
    while True:
        try:
            ...
            if response:
                ...
                if isinstance(response_ok, list):
